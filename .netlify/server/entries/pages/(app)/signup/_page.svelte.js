...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.form === void 0 && $$bindings.form && form !== void 0)
    $$bindings.form(form);
  return `<section class="${"columns mt-6 pt-6"}"><div class="${"column is-half is-offset-one-quarter"}"><h1 class="${"title"}">Sign up</h1>
		${(form == null ? void 0 : form.error) ? `<div class="${"block notification is-danger"}">${escape(form.error)}</div>` : ``}
		${(form == null ? void 0 : form.message) ? `<div class="${"block notification is-primary"}">${escape(form.message)}</div>` : ``}
		...
		<div class="${"mt-6"}"><p class="${"has-text-centered"}">Already have an account? <a href="${"/"}">Sign in</a></p></div></div></section>`;
});
export {
  Page as default
};
